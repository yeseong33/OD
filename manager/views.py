        ...
        if request_type == 'create_graph':
            ...
            def request_and_download_image(image_index, retry_count=0):
                ...
                try:
                    ...
                    filename = os.path.join('static', 'images', f'Redraw_image_{image_index}.png')
                    ...
                    if retry_count < MAX_RETRIES:
                        ...
                        time.sleep(1.5)
                        request_and_download_image(image_index, retry_count + 1)
                    else:
                        print(f"Maximum retry attempts for image {image_index} reached. Aborting.")

            def make_parallel_requests():
                with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                    futures = [executor.submit(request_and_download_image, i) for i in range(3)]
            ...
            new_image_path = os.path.join('static', 'images', f'Redraw_image_{image_number}.png')

            # 기존 이미지 경로 설정
            existing_image_path = os.path.join('static', 'images', 'origin_image.png')
